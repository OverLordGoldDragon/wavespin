...
def compute_meta_jtfs(J_pad, J, Q, T, r_psi, sigma0, average, average_global,
    ...
    def _fill_n1_info(pair, n2, n1_fr, spin):
        if _exclude_excess_scale(n2, n1_fr):
            return
        if (n2 in paths_exclude.get('n2', {}) or
    ...
    if scf.out_3D:
      ...
      for field in array_fields:
        ...
        for pair in meta[field]:
          if 'psi_t' in pair:
              number_of_n2 = sum((j2 != 0 and
                                  n2 not in paths_exclude.get('n2', {}))
                                 for n2, j2 in enumerate(j2s))
          else:
              number_of_n2 = 1
          ...
          if pair in ('S0', 'S1'):
              ...
              if scf.sampling_psi_fr != 'exclude':
                  number_of_n1_fr = len(j1_frs[0])
              else:
                  n_slices = 0
                  if pair.startswith('phi_t'):
                      n2 = -1
                      for n1_fr, j1_fr in enumerate(j1_frs[0]):
                          if _exclude_excess_scale(n2, n1_fr):
                              continue
                          n_slices += 1
                  else:
